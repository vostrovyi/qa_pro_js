...
//Оператори*************************** - https://w3schoolsua.github.io/js/js_operators.html
/**
...
/*var undef;
var n = null; // Під час визначення змінної, яка пізніше призначена для зберігання об’єкта, бажано ініціалізувати змінну значенням null, а не просто залишати undefined
 ...
 */
  ...
  // Повертає перше ІСТИННЕ значення або останнє значення, якщо всі хибні
// console.log(5 || "текст")     // 5 (поверне 5, бо це перше істинне значення)
// console.log(0 || "текст")     // "текст" (0 - хибне, тому поверне "текст")
// console.log("" || "текст")    // "текст" (порожній рядок - хибне значення)
// console.log(0 || "")          // "" (всі хибні, поверне останнє)

// Повертає перше ХИБНЕ значення або останнє значення, якщо всі істинні
// console.log(5 && "текст")     // "текст" (обидва істинні, поверне останнє)
// console.log(0 && "текст")     // 0 (поверне перше хибне значення)
// console.log("" && "текст")    // "" (поверне перше хибне значення)
// console.log(5 && 10)          // 10 (обидва істинні, поверне останнє)
...
// Для var res = false | n  першим кроком відбувається перетворення даних до 32-бітних цілих чисел
// Давайте розберемо перетворення різних типів даних до 32-бітних цілих чисел в JavaScript (ToInt32):
// Undefined
// Спочатку перетворюється в NaN
// NaN перетворюється в +0
...
// -Infinity → 0
// Для інших чисел:
// Спочатку відбувається приведення до 32-бітного цілого числа
// Якщо число виходить за межі [-2^31, 2^31-1], застосовується операція modulо 2^32


// String
// Порожній рядок "" → 0
// Якщо рядок містить число, воно перетворюється за правилами Number
// Якщо рядок не може бути перетворений в число → 0


// Object
// Викликається метод valueOf()
// Якщо результат примітив - конвертується за правилами вище
// Якщо не примітив - викликається toString()
// Отриманий рядок конвертується за правилами String
...
// false | n: Якщо n істинне, результат буде true, в іншому випадку воно буде перетворено в число (0 або 1) і виконається побітове АБО - https://w3schoolsua.github.io/js/js_bitwise.html#gsc.tab=0.
// Результат першого кроку || x: Якщо x істинне, результат буде true, в іншому випадку воно буде перетворено в число і виконається побітове АБО.
// Результат другого кроку || 'Просто рядок': Якщо 'Просто рядок' істинне, результат буде 'Просто рядок', в іншому випадку він буде повернений як 'Просто рядок', оскільки це останній операнд.
// Результат третього кроку || (x = 5): Присвоюємо x значення 5, потім оператор поверне 5 як результат (значення після присвоювання).
// Результат четвертого кроку | true: Здійснюється побітове АБО з true. Результат буде true, оскільки будь-яке число, окрім 0, повертає true в логічному контексті.
